...
<xsl:stylesheet
  ...
  <xsl:template match="//pom:developers">
    <h2><xsl:text>Apache Velocity Commiters</xsl:text></h2>
    <table class="standard">
      ...
      <tbody>
        <xsl:apply-templates select="pom:developer[count(pom:roles/pom:role[contains(text(),'developer')])=1]"/>
      </tbody>
    </table>
  </xsl:template>
  <xsl:template match="//pom:developer">
    <tr>
      ...
      <td><xsl:value-of select="pom:role[contains(text(),'developer')]/text()"/></td>
    </tr>
  </xsl:template>
